...
namespace Book.DAL
{
	...
	public partial class T_STOCK_OutBody
	{
		...
		public int Add(Book.Model.T_STOCK_OutBody model)
		{
			...
			strSql.Append("insert into T_STOCK_OutBody(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update T_STOCK_OutBody set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from T_STOCK_OutBody ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		public Book.Model.T_STOCK_OutBody GetModel(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,BookID,Discount,Num,Total,Price,InHeadID from T_STOCK_OutBody ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			Book.Model.T_STOCK_OutBody model=new Book.Model.T_STOCK_OutBody();
		...
		/// 得到一个对象实体
		/// </summary>
		public Book.Model.T_STOCK_OutBody DataRowToModel(DataRow row)
		{
			Book.Model.T_STOCK_OutBody model=new Book.Model.T_STOCK_OutBody();
			if (row != null)
			{
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM T_STOCK_OutBody ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM T_STOCK_OutBody ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from T_STOCK_OutBody T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "T_STOCK_OutBody";
        ...
        public bool DeleteStr(string p)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from T_Stock_OutBody ");
            strSql.Append(" where InHeadID in (" + p + ")  ");
            int rows = DbHelperSQL.ExecuteSql(strSql.ToString());
            if (rows > 0)
            {
                return true;
        ...
        public bool DeleteByHeadID(int p)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from T_STOCK_OutBody ");
